...
def build_sensor_graph(
    ...
    """
    Build adjacency matrix for EEG sensors
    ...
    if use_correlation:
        ...
        # Keep top-k correlations per node
        topk_vals, topk_idx = torch.topk(corr.abs(), k=k, dim=-1)
        
        # Create sparse adjacency
        for b in range(B):
            for c in range(C):
                A[b, c, topk_idx[b, c]] = topk_vals[b, c]
...
class FusionAlphaGNN(nn.Module):
        ...
        for i in range(n_layers):
            ...
            if use_attention:
                layer = GraphAttentionLayer(
                    in_dim=in_dim,
                    out_dim=hidden_dim // 4 if i < n_layers - 1 else hidden_dim,
                    n_heads=4,
                    concat=True if i < n_layers - 1 else False
                )
            else:
                layer = GCNLayer(in_dim=in_dim, out_dim=hidden_dim)
    ...
    def forward(
        ...
        if mc_samples > 1:
            uncertainty = outputs.var(dim=0)  # [B, output_dim]
        else:
...
class ContradictionResolver(nn.Module):
    ...
    def forward(
        ...
        # Detect contradictions between connected nodes
        contradictions = torch.zeros(B, N, N, device=H.device)
        
        for i in range(N):
            for j in range(i+1, N):
                if A[:, i, j].mean() > 0.1:  # Connected nodes
                    # Check for contradiction
                    pair_feat = torch.cat([H[:, i], H[:, j]], dim=-1)
                    contra_score = self.detector(pair_feat).squeeze(-1)
                    contradictions[:, i, j] = contra_score
                    contradictions[:, j, i] = contra_score
